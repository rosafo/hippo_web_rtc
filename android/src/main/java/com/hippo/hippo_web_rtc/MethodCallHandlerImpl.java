...
import static com.hippo.hippo_web_rtc.utils.MediaConstraintsUtils.parseMediaConstraints;
...
import com.hippo.hippo_web_rtc.audio.AudioDeviceKind;
import com.hippo.hippo_web_rtc.audio.AudioSwitchManager;
import com.hippo.hippo_web_rtc.record.AudioChannel;
import com.hippo.hippo_web_rtc.record.FrameCapturer;
import com.hippo.hippo_web_rtc.utils.AnyThreadResult;
...
public class MethodCallHandlerImpl implements MethodCallHandler, StateProvider {
 ...
 // CameraCharacteristics cameraCharacteristics = cameraManager.getCameraCharacteristics(CIMO_8M_CAMERA_ID);
  ...
  MethodCallHandlerImpl(Context context, BinaryMessenger messenger, TextureRegistry textureRegistry,
    ...
    this.cameraManager = (CameraManager) this.context.getApplicationContext().getSystemService(Context.CAMERA_SERVICE);
  }
  ...
  private void ensureInitialized() {
    ...
    mFactory = PeerConnectionFactory.builder()
            ...
            // this was commented because SimulcastVideoEncoderFactoryWrapper could not be found and referenced
         //   .setVideoEncoderFactory(new SimulcastVideoEncoderFactoryWrapper(eglContext, true, true))
            // so i replaced it with DefaultVideoEncoderFactory for now
  ...
  public void onMethodCall(MethodCall call, @NonNull Result notSafeResult) {
    ...
    switch (call.method) {
      ...
      case "mediaStreamTrackSwitchCamera": {
        String trackId = call.argument("trackId");
        Log.i("switchCamera", "ID received 1 = " + trackId);
        try {
          getUserMediaImpl.switchCamera(trackId, result);
        } catch (Exception e) {
          e.printStackTrace();
        }
        Log.i("switchCamera", "deviceNames before break");
        break;
      }
      case "setVolume": {
        String trackId = call.argument("trackId");
      ...
      case "isZoomSupported" : {
        String trackId = call.argument("trackId");
        getUserMediaImpl.isZoomSupported(trackId, result);
        break;
      }
      case "setZoom" : {
        int zoom = call.argument("zoom");
        String trackId = call.argument("trackId");
        getUserMediaImpl.setZoom(trackId, result, zoom);
        break;
      }
      case "setZoom1" : {
        String trackId = call.argument("trackId");
        getUserMediaImpl.setZoom1(trackId, result);
        break;
      }
      case "setZoom2" : {
        String trackId = call.argument("trackId");
        getUserMediaImpl.setZoom2(trackId, result);
        break;
      }
      case "setZoom3" : {
        String trackId = call.argument("trackId");
        getUserMediaImpl.setZoom3(trackId, result);
        break;
      }
      case "setZoom4" : {
        String trackId = call.argument("trackId");
        getUserMediaImpl.setZoom4(trackId, result);
        break;
      }
      case "setZoom5" : {
        String trackId = call.argument("trackId");
        getUserMediaImpl.setZoom5(trackId, result);
        break;
      }
      case "getZoom" : {
        String trackId = call.argument("trackId");
        getUserMediaImpl.getZoom(trackId, result);
        break;
      }
      case "getMaxZoom" : {
        String trackId = call.argument("trackId");
        getUserMediaImpl.getMaxZoom(trackId, result);
        break;
      }
      case "setLightOn" : {
        String trackId = call.argument("trackId");
        getUserMediaImpl.setLightOn(trackId, result);
        break;
      }
      case "setLightOff" : {
        String trackId = call.argument("trackId");
        getUserMediaImpl.setLightOff(trackId, result);
        break;
      }
      case "getLightStatus" : {
        String trackId = call.argument("trackId");
        getUserMediaImpl.getLightStatus(trackId, result);
        break;
      }
      //
      case "turnLightOn" : {
        String trackId = call.argument("trackId");
        getUserMediaImpl.turnLightOn(trackId, result);
        break;
      }
      case "turnLightOn1" : {
        String trackId = call.argument("trackId");
        int a = call.argument("a");
        getUserMediaImpl.turnLightOn1(trackId, a ,result);
        break;
      }

      case "turnLightOff" : {
        String trackId = call.argument("trackId");
        getUserMediaImpl.turnLightOff(trackId, result);
        break;
      }
      case "turnLightOff1" : {
        String trackId = call.argument("trackId");
        int a = call.argument("a");
        getUserMediaImpl.turnLightOff1(trackId, a ,result);
        break;
      }

      case "turnLightStatus" : {
        String trackId = call.argument("trackId");
        getUserMediaImpl.turnLightStatus(trackId, result);
        break;
      }
      case "turnLightStatus1" : {
        String trackId = call.argument("trackId");
        int a = call.argument("a");
        getUserMediaImpl.turnLightStatus1(trackId, a ,result);
        break;
      }
  ...
  public void getSources2(Result result) throws Exception {
    ConstraintsArray array = new ConstraintsArray();
    String[] ids = cameraManager.getCameraIdList();
    String[] names = new String[ids.length];

    Log.i("getSources2", "cameraManager.getCameraIdList() size = " + ids.length);
    Log.i("getSources2", "cameraManager.getCameraIdList() value = " + ids.toString());

    for (int i = 0; i < ids.length; ++i) {
      ConstraintsMap info = getCameraInfo2(i, ids[i]);
      if (info != null) {
        array.pushMap(info);
      }
    }
  ...
  public ConstraintsMap getCameraInfo2(int index, String cameraId) throws CameraAccessException {
    CameraCharacteristics chars = cameraManager.getCameraCharacteristics(cameraId);
    Integer facingInt = chars.get(CameraCharacteristics.LENS_FACING);

    ConstraintsMap params = new ConstraintsMap();
   // String facing = facingInt == 1 ? "front" : "back";
    String facing = cameraId.equals("1") ? "front" : "back";
    Log.i("getCameraInfo2", "index = " + index + " cameraId = " + cameraId + " facing = " + facing);
    params.putString("label",
            "Camera " + index + ", Facing " + facing + ", Orientation " + facingInt);
    params.putString("deviceId", "" + index);
    params.putString("facing", facing);
    params.putString("kind", "videoinput");
    return params;
